		...
		var mdater = {
			...
			refreshView : function(){
				...
				if(initVal){
					...
					date = new Date(arr[0], arr[1]-1 , "01");
				}
				else{
					date = new Date();
				}
			...
			initListeners : function(){
				var panel = $(option.outercontainer).find('.md_panel'),
					mask = $(option.outercontainer).find('.md_mask'),
					_this = this;
				...
				panel.delegates({
					...
					'.md_ok' : function(){
						...
						input.val(_this.value.year + '-' + monthValue);
						_this._hidePanel();
						option.okcallback();
					},
					'.md_cancel' : function(){
